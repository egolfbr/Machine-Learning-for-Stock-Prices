...
I learn by getting my hands dirty and by trying examples and experimenting with the libraries and code available. This is why I did minimum research before jumping in and trying to build a model. It was probably a little naive of me to think that I would be able to produce a model with even remotely good results and that of course didn't happen. Thus, I went back to reading and research and found another avenue to experiment with: Polynomial regression. So far my trials have consisted of linear regression and sequential neural networks. However, my data is not linear in nature. There are many variables and relationships that affect the price of a stock. The goal of this project is to create a more general machine learning algorithm that can be applied to any ticker symbol supplied. This is why polynomial regression might not be the best end solution, but is something that I am going to try. I say that it might not work as an end solution because polynomial regression will try and find coefficents to a polynomial whose degree is greater than 1, which means it will find one equation. If I train the model on Microsoft data, it will be a poor predicter of Apple data. It might work as an end solution if I add another step in the data pipeline where after you input a ticker, the model will be trained first on that tickers training data. However, another deliverable is to make this algorithm as fast as possible. In the world of investing, milliseconds can mean the difference between making money and losing money. 
